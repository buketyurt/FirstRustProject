...
thread_local! {
 ...
 fn get_proposal(key: u64) -> Option<Proposal> {
    return PROPOSAL_MAP.with{|p: &RefCell<StableBTreeMap<u64,Proposal,VirtualMemory<std::rc::Rc<RefCell<Vec<u8>>>>>>| p.borrow().get(&key)};
 }

 #[ic_cdk::query]
 fn get_proposal_count() -> u64 {
    return PROPOSAL_MAP.with{|p: &RefCell<StableBTreeMap<u64,Proposal,VirtualMemory<std::rc::Rc<RefCell<Vec<u8>>>>>>| p.borrow().len()};
 }
 ...
 fn create_proposal(key: u64, proposal: CreateProposal) -> Option<Proposal> {
    ...
    PROPOSAL_MAP.with{|p : &RefCell<StableBTreeMap<u64,Proposal,VirtualMemory<std::rc::Rc<RefCell<Vec<u8>>>>>>| p.borrow_mut().insert(key,value)}
 }

 #[ic_cdk::update]
 fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    PROPOSAL_MAP.with{|p|{
        let old_proposal_opt= p.borrow().get(&skey);
 ...
 fn vote(key: u64, choice: Choice) -> Result<(), VoteError> {
    ...
    let res: RefMute<'_,BTreeMap<u64,_,_>> = p.borrow_mut().insert(key,proposal);
